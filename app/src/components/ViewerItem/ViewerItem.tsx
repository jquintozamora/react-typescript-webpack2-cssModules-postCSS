...
class ViewerItem extends React.Component<IViewerItemProps, {}> {
  public render() {
    ...
    const finalSummary: string =
      summary.length > summaryMaxWords
        ? summary.substring(0, summaryMaxWords).concat("...")
        : summary;
    const titleMaxWords: number = typeSingleton.titleMaxWords;
    const finalTitle: string =
      title.length > titleMaxWords ? title.substring(0, titleMaxWords).concat("...") : title;

    // Get JS Styles
    const InlineStyles: any =
      typeof ViewerItemCardTypeStyles === "function" && ViewerItemCardTypeStyles(typeSingleton);
        ...
        <tbody>
          ...
          <tr>
            <td style={InlineStyles && InlineStyles.title.td}>
              <a target="_blank" style={InlineStyles && InlineStyles.title.a} href={linkUrl}>
                {finalTitle}
              </a>
            </td>
          </tr>
          <tr>
            <td style={InlineStyles && InlineStyles.date}>{date}</td>
          </tr>
          <tr>
            <td style={InlineStyles && InlineStyles.summary}>{finalSummary}</td>
          </tr>
        </tbody>
      </table>
    );
