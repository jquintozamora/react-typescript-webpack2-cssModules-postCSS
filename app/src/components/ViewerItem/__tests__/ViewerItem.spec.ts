...
describe("<ViewerItem />", () => {
  const initialProps = {
    ...
    typeSingleton: ViewerItemCardType.Small
  };
  const renderComponent = testComponentHelper(ViewerItem);
  describe("@renders", () => {
    it("in default state", () => {
      expect(renderComponent({ ...initialProps } as any).getHtml()).toMatchSnapshot();
    });
  });
});
